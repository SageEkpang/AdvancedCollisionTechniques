...
BoxCollider::BoxCollider(float x_scale, float y_scale, float z_scale)
{
	m_Offset = Vector3(0, 0, 0);
	m_Scale = Vector3(x_scale, y_scale, z_scale);
}
...
void BoxCollider::Construct(Vector3 offset, Vector3 scale, ID3D11Device* device)
{
	m_Offset = offset;
	m_Scale = scale;
...
void BoxCollider::Construct(Vector3 offset, float width, float height, float length, ID3D11Device* device)
{
	m_Offset = offset;
	m_Scale = Vector3(width, height, length);
...
void BoxCollider::Construct(float x_scale, float y_scale, float z_scale, ID3D11Device* device)
{
	m_Offset = Vector3(0, 0, 0);
	m_Scale = Vector3(x_scale, y_scale, z_scale);
...
void BoxCollider::Construct(Vector3 scale, ID3D11Device* device)
{
	m_Offset = Vector3(0, 0, 0);
	m_Scale = scale;
...
void BoxCollider::Construct(float x_offset, float y_offset, float z_offset, float width, float height, float length, ID3D11Device* device)
{
	m_Offset = Vector3(x_offset, y_offset, z_offset);
	m_Scale = Vector3(width, height, length);
