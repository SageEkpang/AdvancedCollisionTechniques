...
namespace CollisionContactManager
{
	void ResolveCollision(GameObjectEntity* gameObjectA, GameObjectEntity* gameObjectB, float CoefRest, CollisionManifold collisionManifold)
	{
			...
			&& gameObjectA->GetComponent<Rigidbody2DComponent>()->GetRigidbodyMovementType() == RIGIDBODY_MOVEMENT_TYPE_DYNAMIC)
		{
			...
			if (collisionManifold.penetrationDepth != 0) { gameObjectA->m_Transform.position += t_MoveOutA; }
			...
			gameObjectA->GetComponent<Rigidbody2DComponent>()->ApplyImpulseZ(impulseZ / gameObjectA->GetComponent<Rigidbody2DComponent>()->m_Mass);
		}
		...
		{
			// NOTE: Seperate the 2 objects away from each other if they are still interpentrating
			if (collisionManifold.penetrationDepth != 0) { gameObjectB->m_Transform.position += t_MoveOutB; }
			...
			gameObjectB->GetComponent<Rigidbody2DComponent>()->ApplyImpulseZ(-1 * impulseY / gameObjectB->GetComponent<Rigidbody2DComponent>()->m_Mass);
		}
	}
